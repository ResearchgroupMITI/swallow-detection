...
class ModelSaving:
    ...
    def __call__(self, metric_dict, model):
        ...
        index_insert = self.save_model_metric.index("val_") + len("val_")
        new_string = self.save_model_metric[:index_insert] + f"fold{self.index}_" + self.save_model_metric[index_insert:]
    ...
    def save_model(self, model):
        ...
        if self.save_best_model:
            torch.save(model.state_dict(), os.path.join(models_path, f"best_model_fold{self.index}.pt"))
